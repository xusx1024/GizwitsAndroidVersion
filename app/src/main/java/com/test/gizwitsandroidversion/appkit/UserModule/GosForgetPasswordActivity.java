...
@SuppressLint("HandlerLeak") public class GosForgetPasswordActivity
    extends GosUserModuleBaseActivity implements OnClickListener {
      ...
      switch (key) {
        ...
        case RESET:
          ...
          GizWifiSDK.sharedInstance()
              .resetPassword(name, code, psw, GizUserAccountType.GizUserPhone);
          break;
      }
    }
  };
    ...
    switch (v.getId()) {
      case R.id.btnGetCode:
        ...
        if (TextUtils.isEmpty(name)) {
          Toast.makeText(GosForgetPasswordActivity.this, R.string.toast_name_wrong, toastTime)
              .show();
          return;
        }
      ...
      case R.id.btnReset:
        ...
        if (TextUtils.isEmpty(psw)) {
          Toast.makeText(GosForgetPasswordActivity.this, R.string.toast_psw_wrong, toastTime)
              .show();
          return;
        }
      /*
  ...
  /** 手机验证码回调 */
  @Override protected void didRequestSendPhoneSMSCode(GizWifiErrorCode result, String token) {
  ...
  /** 重置密码回调 */
  @Override public void didChangeUserPassword(GizWifiErrorCode result) {
    progressDialog.cancel();
    Message msg = new Message();
    if (GizWifiErrorCode.GIZ_SDK_SUCCESS != result) {
