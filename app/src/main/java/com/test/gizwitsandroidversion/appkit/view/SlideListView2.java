package com.test.gizwitsandroidversion.appkit.view;
 ...
 * @author zhangshuo
 */
@SuppressLint({ "ClickableViewAccessibility", "Recycle" }) public class SlideListView2
    extends ListView {
  ...
  @Override public boolean onTouchEvent(MotionEvent ev) {
    ...
    switch (action) {
      ...
      case MotionEvent.ACTION_MOVE:
        ...
        if (!canMove && slidePosition != AdapterView.INVALID_POSITION && (Math.abs(
            ev.getX() - downX) > mTouchSlop && Math.abs(ev.getY() - downY) < mTouchSlop)) {
