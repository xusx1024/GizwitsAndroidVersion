...
public class GosChangeUserPasswordActivity extends GosBaseActivity {
  ...
  GizWifiSDKListener listener = new GizWifiSDKListener() {
    ...
    @SuppressLint("WrongConstant") public void didChangeUserPassword(com.gizwits.gizwifisdk.enumration.GizWifiErrorCode result) {

      if (result.getResult() == 0) {
        Toast.makeText(GosChangeUserPasswordActivity.this,
            getResources().getString(R.string.passsuccess), 2000).show();

        finish();

        spf.edit().putString("PassWord", newpass.getText().toString()).commit();
      } else {

        if (result.getResult() == 9020) {
          Toast.makeText(GosChangeUserPasswordActivity.this,
              getResources().getString(R.string.oldpasserror), 2000).show();
        } else {
          Toast.makeText(GosChangeUserPasswordActivity.this,
              getResources().getString(R.string.passerror), 2000).show();
        }
      }
    }
  ...
  private void initView() {
    ...
    confrimpass.setTransformationMethod(PasswordTransformationMethod.getInstance());
    ...
    concheck.setOnCheckedChangeListener(new OnCheckedChangeListener() {
      ...
      @Override public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
        ...
        if (isChecked) {
          ...
          confrimpass.setTransformationMethod(HideReturnsTransformationMethod.getInstance());
        } else {
  ...
  @SuppressLint("WrongConstant") public void confirm(View v) {

    if (TextUtils.isEmpty(oldpass.getText().toString()) || TextUtils.isEmpty(
        newpass.getText().toString()) || TextUtils.isEmpty(confrimpass.getText().toString())) {

      if (TextUtils.isEmpty(oldpass.getText().toString())) {

        Toast.makeText(this, getResources().getString(R.string.enter_current_password), 2000)
            .show();
      ...
      if (TextUtils.isEmpty(newpass.getText().toString())) {
        Toast.makeText(this, getResources().getString(R.string.enter_new_password), 2000).show();

        return;
      }

      if (TextUtils.isEmpty(confrimpass.getText().toString())) {
        Toast.makeText(this, getResources().getString(R.string.re_enter_new_password), 2000).show();
        return;
      }
    } else {
      ...
      if (npass.equals(cpass)) {
        GizWifiSDK.sharedInstance()
            .changeUserPassword(spf.getString("Token", ""), oldpass.getText().toString(), npass);
      } else {
        Toast.makeText(this, getResources().getString(R.string.nosamepass), 2000).show();
      }
    }
  }
