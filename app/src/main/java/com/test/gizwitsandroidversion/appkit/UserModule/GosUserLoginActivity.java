package com.test.gizwitsandroidversion.appkit.UserModule;
...
import com.test.gizwitsandroidversion.appkit.DeviceModule.GosDeviceListActivity;
import com.test.gizwitsandroidversion.appkit.DeviceModule.GosMainActivity;
import com.test.gizwitsandroidversion.appkit.GosApplication;
import com.test.gizwitsandroidversion.appkit.PushModule.GosPushManager;
import com.test.gizwitsandroidversion.appkit.ThirdAccountModule.BaseUiListener;
import com.test.gizwitsandroidversion.appkit.view.DotView;
import java.util.Timer;
import java.util.TimerTask;
import org.json.JSONException;
import org.json.JSONObject;

@SuppressLint("HandlerLeak") public class GosUserLoginActivity extends GosUserModuleBaseActivity
      ...
      switch (key) {
        ...
        case LOGIN:
          ...
          GizWifiSDK.sharedInstance()
              .userLogin(etName.getText().toString(), etPsw.getText().toString());
          break;
        // 自动登录
        case AUTO_LOGIN:
          progressDialog.show();
          GosDeviceListActivity.loginStatus = 0;
          GizWifiSDK.sharedInstance()
              .userLogin(spf.getString("UserName", ""), spf.getString("PassWord", ""));
          break;
        // 第三方登录
        case THRED_LOGIN:
          progressDialog.show();
          GosDeviceListActivity.loginStatus = 0;
          GizWifiSDK.sharedInstance()
              .loginWithThirdAccount(gizThirdAccountType, thirdUid, thirdToken);
    ...
    switch (v.getId()) {
      ...
      case R.id.llQQ:
        ...
        if (TextUtils.isEmpty(tencentAPPID) || tencentAPPID.contains("your_tencent_app_id")) {
          ...
          // 启动QQ登录SDK
          mTencent =
              Tencent.createInstance(GosDeploy.setTencentAppID(), this.getApplicationContext());
        }
   ...
   * 用户登录回调
   */
  @Override protected void didUserLogin(GizWifiErrorCode result, String uid, String token) {
    ...
    if (GizWifiErrorCode.GIZ_SDK_SUCCESS != result) {// 登录失败
      ...
      if (!TextUtils.isEmpty(etName.getText().toString()) && !TextUtils.isEmpty(
          etPsw.getText().toString()) && TextUtils.isEmpty(spf.getString("thirdUid", ""))) {
  ...
  @Override protected void didNotifyEvent(GizEventType eventType, Object eventSource,
