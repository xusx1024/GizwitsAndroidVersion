...
import com.test.gizwitsandroidversion.appkit.utils.HexStrUtils;
...
public class GosControlModuleBaseActivity extends GosBaseActivity {
   ...
   */
  // 数据点"Swtich"对应的标识名
  protected static final String KEY_SWTICH = "onoff";
  protected static final String KEY_RED = "r";
  protected static final String KEY_GREEN = "g";
  protected static final String KEY_BLUE = "b";
   ...
   */
  // 数据点"Swtich"对应的存储数据
  protected static boolean data_Swtich;
  protected static int data_red;
  protected static int data_green;
  protected static int data_blue;
  GizWifiDeviceListener gizWifiDeviceListener = new GizWifiDeviceListener() {

    /** 用于设备订阅 */
    public void didSetSubscribe(GizWifiErrorCode result, GizWifiDevice device,
  ...
  private <T> T getDataFromKey(ConcurrentHashMap<String, Object> map, String key) {
  ...
  protected void getDataFromReceiveDataMap(ConcurrentHashMap<String, Object> dataMap) {
    ...
    if (dataMap.get("data") != null) {
      ...
      for (String dataKey : map.keySet()) {
        switch (dataKey) {
          case KEY_SWTICH:
            data_Swtich = getDataFromKey(map, dataKey);
            break;
          case KEY_RED:
            data_red = getDataFromKey(map, dataKey);
            break;
          case KEY_GREEN:
            data_green = getDataFromKey(map, dataKey);
            break;
          case KEY_BLUE:
            data_blue = getDataFromKey(map, dataKey);
            break;
        }
      }
   ...
   */
  protected void didSetSubscribe(GizWifiErrorCode result, GizWifiDevice device,
      boolean isSubscribed) {
  }
